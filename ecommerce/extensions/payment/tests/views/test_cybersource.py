...
class CybersourceSubmitViewTests(CybersourceMixin, TestCase):
    ...
    @ddt.unpack
    def test_submit_view_fails_for_invalid_voucher(self, end_time, status_code):
        """ Verify SubmitPaymentView fails if basket invalid voucher"""
        # Create Basket and payment data
        basket = self._prepare_basket_for_voucher_validation_tests(now() - datetime.timedelta(days=5), end_time)
    ...
    def test_submit_view_fails_if_voucher_not_available(self, mock_is_available_to_user):
        ...
        # Create Basket and payment data
        basket = self._prepare_basket_for_voucher_validation_tests(now() - datetime.timedelta(days=1),
    ...
    def test_successful_submit_view_with_voucher_switch_disabled(self):
        """
        Verify SubmitPaymentView call is successful if voucher waffle switch disabled
        and expired voucher is applied. """
        basket = self._prepare_basket_for_voucher_validation_tests(now() - datetime.timedelta(days=5),
                                                                   now() - datetime.timedelta(days=3))
        data = self._generate_data(basket.id)
        response = self.client.post(self.path, data)
